...
const REASONS = {
  NO_PERIODS: "no_periods", // qator umuman yo'q → abadiy o'qiydi
 ...
 * filtrlab yubormasligi SHART: aks holda o'tgan yili ketgan o'quvchi ham
 * bo'sh massiv beradi va "abadiy o'qiydi" deb hisoblanib, unga abadiy
 * hisob-faktura yozilardi.
 *
...
function resolveEnrollmentForMonth(periods, monthKey) {
  ...
  // Qator yo'q → orqaga moslik: tizimga o'tishdan oldingi o'quvchilar
  if (!periods || periods.length === 0) return full(REASONS.NO_PERIODS);
 ...
 * Davr umuman bo'lmasa ham "o'qiyapti" (orqaga moslik).
 *
 * @param {Array<{startDate: Date, endDate: Date|null}>} periods
...
function describeEnrollment(periods = []) {
  if (periods.length === 0) {
    return { isStudying: true, since: null, until: null, hasPeriods: false };
  }
